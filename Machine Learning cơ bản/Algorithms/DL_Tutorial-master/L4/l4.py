...
class NeuralNetwork:
    ...
    def calculate_loss(self, X, y):
        ...
        return -( np.sum( y*np.log(y_predict) + (1-y)*np.log(1-y_predict) ) ), y_predict

    def interpret_predict(self, predict):
        n = len(predict)
        predict_class =  np.zeros(n,)
        for i in range(n):
            if predict[i] >= 0.5:
                predict_class[i] = 1
    ...
    def fit(self, X, y, epochs=20, verbose=10):
        for epoch in range(0, epochs):
            ...
            if epoch % verbose == 0:
                loss, predict = self.calculate_loss(X, y)
                predicted_class = self.interpret_predict(predict)
                print("Epoch {}, loss {}".format(epoch, loss))
                print('training accuracy: %.2f' % (np.mean(predicted_class == y.T)))
                #print(y.T)
